...
user_details = JSON.parse(localStorage.getItem("JSON-Program")) ? JSON.parse(localStorage.getItem("JSON-Program")) : []
...
let photo = new Array();
photo = JSON.parse(localStorage.getItem("userphoto")) ? JSON.parse(localStorage.getItem("userphoto")) : []
...
if(localStorage.getItem('posts') == null){
    ...
    .then((res) => res.json())
    .then((data) => {
        let postnum = data.slice(0,10);
        for(var i in postnum){
            let id= postnum[i].id;
            let userId = postnum[i].userId;
            let title = postnum[i].title;
            let body = postnum[i].body;
            post.push({
                "userId" : userId,
                "id" : id,
                "title" : title,
...
output1 = JSON.parse(localStorage.getItem('finall')) ? JSON.parse(localStorage.getItem('finall')) : []
output1 = user_details.map((item,i) => Object.assign({},item,post[i]));
localStorage.setItem('finall',JSON.stringify(output1));
     function showApiData(){
        let users_details = new Array();
        users_details = JSON.parse(localStorage.getItem("arr")) ? JSON.parse(localStorage.getItem("arr")) : []
        if(users_details){
            for (var i in users_details) {
                output += `
                            ...
                            &nbsp;<b>Name:</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span <h5 class="card-title">${users_details[i].name}</h5> <br>
                            ...
                            <img src="${users_details[i].thumbnailUrl}" onclick="showphoto(${i})" height=40px; width=40px; data-target="#example"/>
                            <a href="${users_details[i].userId}" onclick="showpost(${i})" class="btn btn-primary" style=" padding: 5px;">Post</a>
                            </div> 
                            </div> <br> `
            }
    ...
    function validate(){
       ...
       document.getElementById('update').style = "display:none"
        ...
        if (latitude1 == '') {
            document.getElementById('latitude').innerHTML = 'Please Enter your latitude';
            return false
        }
        let longitude1 = document.getElementById('longitude').value;
        if (longitude1 == '') {
            document.getElementById('longitude').innerHTML = 'Please Enter your longitude';
            return false
        } 
        user_details = JSON.parse(localStorage.getItem("JSON-Program")) ? JSON.parse(localStorage.getItem("JSON-Program")) : [] 
        let exist = user_details.find((element) => {return element.email==document.getElementById('email').value.toLowerCase()});
        if(!exist) {
            user_details.push({
                "name": names.value,
...
function showpost(index){
    var url = JSON.parse(localStorage.getItem('finall'))[index].body
    document.getElementById('userid').href = url;
